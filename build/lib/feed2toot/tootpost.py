...
class TootPost:
    ...
    def __init__(self, config, options, toot, spoiler_text):
        '''Constructore of the TootPost class'''
        self.config = config
        self.options = options
        self.store = True
        self.toot = toot
        # option for Content Warning
    ...
    def main(self):
        ...
        use_cw = self.config.get('mastodon', 'use_cw', fallback='Y')
        if 'custom' in self.options['media']:
            ...
            if use_cw == 'Y':
                mastodon.status_post(self.toot, visibility=toot_visibility, spoiler_text=self.spoiler_text)
            else:
                self.toot = 'Using CW, spoiler text: {}'.format(self.spoiler_text) + '\n' + self.toot
