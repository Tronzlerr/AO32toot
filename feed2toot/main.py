...
class Main:
    ...
    def main(self):
        ...
        confs = cfgp.confvalues # List[Tuple]
        for conf in confs:
            options = conf[0]
            config = conf[1]
            tweetformat = conf[2]  # e.g., {title} {link}
            feeds = conf[3]
            plugins = conf[4]
            ...
            for feed in feeds:
                ...
                # extract ao3 tag (the one that is being subscribed) from rss
                # should be something like "AO3 works tagged 'Sam Flynn/Tron'"
                # will use this as spoiler text (cw)
                main_tag = feed['feed']['feed']['title'].split("'")[1]
                spoiler_title = "以下是AO3标签'{}'中的新增作品".format(main_tag)
                ...
                for i, entry in enumerate(totweet):
                    spoiler_text = "{} ({}/{})".format(spoiler_title, i+1, len(totweet))
                    ...
                    if entrytosend:
                        ...
                        if clioptions.dryrun:
                            send_message_dry_run(config, entrytosend, finaltweet, spoiler_text)
                        else:
                            send_message(config, clioptions, options, entrytosend, finaltweet, spoiler_text, cache, rss)
                            # plugins
